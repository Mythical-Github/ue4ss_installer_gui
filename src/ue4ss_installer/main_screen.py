...
from ue4ss_installer import web, add_game_screen, steam, epic, unreal_engine


scroll_area_height = window_height - (HEADER_HEIGHT + SUBHEADER_HEIGHT + FOOTER_HEIGHT + DIVIDER_HEIGHT + MARGIN) - 60
...
def init_main_screen_header():
    with dpg.group(horizontal=True):
        ...
        dpg.add_spacer(width=(window_width - title_width) // 2)
        dpg.add_text(title, tag="HeaderText")
        dpg.bind_item_font("HeaderText", "header_font")


def init_main_screen_sub_header():
    subheader_text = "     To install UE4SS, choose one of the games below, or add a game manually"
...
def init_main_screen_game_list_scroll_box():
    with dpg.child_window(width=-1, height=scroll_area_height, tag="GameListScroll", autosize_x=True):
        # make sure that even games from settings files don't cause double entires from auto detected entires
        all_game_dirs = [
            game
            for dir_source in (steam.get_all_steam_game_directories(), epic.get_all_epic_games_game_directories())
            for base_dir in dir_source
            for game in unreal_engine.get_all_unreal_game_directories_in_directory_tree(str(base_dir))
        ]
...
def init_main_screen_footer_section():
    with dpg.child_window(width=-1, height=FOOTER_HEIGHT, autosize_x=True, autosize_y=True, border=False):
        with dpg.group(horizontal=True):
            with dpg.group(horizontal=True):
                discord_button = dpg.add_button(label="Docs", width=60, height=30)
                dpg.set_item_callback(discord_button, lambda: web.open_website("https://docs.ue4ss.com/"))

                discord_button = dpg.add_button(label="Discord", width=60, height=30)
                dpg.set_item_callback(discord_button, lambda: web.open_website("https://discord.com/invite/7qhRGHF9Tt"))
                
                github_button = dpg.add_button(label="Github", width=60, height=30)
                dpg.set_item_callback(github_button, lambda: web.open_website("https://github.com/UE4SS-RE/RE-UE4SS"))
            
            dpg.add_spacer(width=window_width - (4 * 50 + 20 + 160) - 24)
...
def push_main_screen():
    with dpg.window(label="UE4SS Installer", tag="MainWindow", no_title_bar=True, width=window_width, height=window_height):
