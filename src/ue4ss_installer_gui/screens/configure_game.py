...
def push_uninstall_successful_screen(user_data):
    notification_screen.push_notification_screen(translator.translator.translate('uninstall_succeeded_message_text'), pathlib.Path(user_data))


def push_install_successful_screen(user_data):
    if isinstance(user_data, list):
        user_data = user_data[0]  
    notification_screen.push_notification_screen(translator.translator.translate('install_succeeded_message_text'), pathlib.Path(user_data))


def push_uninstall_failed_screen(user_data):
    notification_screen.push_notification_screen(translator.translator.translate('uninstall_failed_message_text'), pathlib.Path(user_data))


def push_install_failed_screen(user_data):
    notification_screen.push_notification_screen(translator.translator.translate('install_failed_message_text'), pathlib.Path(user_data))
...
def refresh_file_to_install_combo_box(user_data):
    ...
    # Apply the filter here to match filenames with the filter value
    ...
    # Set the default value based on the filtered list
    if filtered_filenames:
        ...
        # If no items match the filter, fallback to the original sorted_filenames
...
# right now things aren't calling this right
        ...
        for file_to_delete in game_info.installed_files:
            ...
            if os.path.isfile(file_to_delete_actual_path):
                try:
                    ...
                    push_uninstall_failed_screen(user_data)
...
def download_ue4ss(user_data):
    try:
        ...
        if game_info:
            ...
            file_names_to_download_links = ue4ss.get_file_name_to_download_links_from_tag(game_info.ue4ss_version)
            download_link = file_names_to_download_links.get(game_info.last_installed_version)
            file_io.download_file(download_link, os.path.normpath(f'{str(file_io.get_temp_dir())}/ue4ss.zip'))
    except:
        push_install_failed_screen(user_data)
...
def push_installing_from_zip_screen(sender, app_data, user_data):
    last_installed_file = dpg.get_value('ue4ss_file_to_install_combo_box') # have this use provided file later
    ue4ss_version = dpg.get_value('tags_combo_box') # have this use provided file later
    update_game_info_field_from_ui(user_data, 'last_installed_version', last_installed_file)
    ...
    if game_info:
        ...
        for game_key in constants.GAME_PATHS_TO_DISPLAY_NAMES.keys():
            if game_key in install_dir:
                centered_game_name_text = f'Game: {str(
        ...
        add_centered_text(f'{translator.translator.translate('game_directory_text_label')} {str(game_info.install_dir)}', parent="configure_game_modal")
        ...
        add_centered_text(translator.translator.translate('ue4ss_version_text_label'), parent="configure_game_modal")
        ...
        dpg.add_input_text(width=-1, hint='filter ue4ss version here...', parent="configure_game_modal", tag='filter_ue4ss_tag', callback=filter_ue4sS_tag, user_data=user_data)
        ...
        add_centered_text(translator.translator.translate('ue4ss_file_to_install_text_label'), parent="configure_game_modal")

        dpg.add_spacer(parent="configure_game_modal")

        dpg.add_input_text(hint='filter file archive to install here...', parent="configure_game_modal", width=-1, tag='filter_ue4ss_file_to_install', callback=filter_ue4ss_file_to_install, user_data=user_data)
        ...
        with dpg.group(horizontal=True, parent="configure_game_modal"):
            ...
            dpg.add_text(translator.translator.translate('install_portable_version_text_label'))
...
# this should check the the related filter input box
