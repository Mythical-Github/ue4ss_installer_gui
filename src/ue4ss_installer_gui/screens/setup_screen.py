...
def push_setup_screen(
    ...
    tags = ["task_text", "step_text", "progress_bar", tag]
    ...
    with dpg.window(
        ...
        dpg.add_button(
            label=f"{task_text}:", tag="task_text", enabled=False, width=-1, height=28
        )
        dpg.add_spacer()
        dpg.add_button(
            label="Starting...", tag="step_text", enabled=False, width=-1, height=28
        )
        dpg.add_spacer()
