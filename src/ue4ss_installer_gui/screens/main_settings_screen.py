...
def push_main_settings_screen():
    ...
    with dpg.window(
        ...
        auto_align.add_multi_line_centered_text("Settings", parent="main_settings_screen")
        ...
        with dpg.group(enabled=settings.get_use_custom_font_from_settings(), tag="font_holder"):
            dpg.add_text(default_value=f"Font path: {settings.get_custom_font_path_from_settings()}")
            dpg.add_button(label="Change font path", width=-1, height=28, callback=change_font_button_selected)
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_checkbox(default_value=default, callback=settings.toggle_use_automatic_game_scanning_in_settings_file)
