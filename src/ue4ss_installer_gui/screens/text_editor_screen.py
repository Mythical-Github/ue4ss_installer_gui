...
def push_text_editor_screen(sender, app_data, user_data):
    ...
    with dpg.window(
        ...
        dpg.add_input_text(tag="text_editor_input", default_value=file_io.get_contents_of_file(user_data['file_path']), multiline=True, width=-1, height=340)
        
        dpg.add_spacer()

        with dpg.group(horizontal=True):
            dpg.add_button(label="Cancel", width=250, height=28, callback=cancel_text_edit_callback, user_data=user_data)
            dpg.add_button(label="Save", width=250, height=28, callback=save_text_edit_callback, user_data=user_data)
