...
def push_configure_game_screen(user_data):
    ...
    if game_info:
        ...
        with dpg.group(
            horizontal=True, parent="configure_game_modal", tag="game_label_container"
        ):
            centered_text.add_centered_text(
                f"{translator.translator.translate('game_label')}: {centered_game_name_text}",
                parent="game_label_container",
            )
...
def refresh_ue4ss_tags_combo_box(user_data):
    ...
    if game_info.ue4ss_version in filtered_tags:
        ...
        dpg.set_value(
            "tags_combo_box",
            filtered_tags[0] if filtered_tags else (all_tags[0] if all_tags else ""),
        )
